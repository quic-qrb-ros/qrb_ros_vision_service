...
{
ObjectDetectExecutor::ObjectDetectExecutor(std::shared_ptr<CallbackManager> & cb_manager,
    const uint8_t camera_id)
  : ExecutorBase(cb_manager, camera_id)
{
}

ObjectDetectExecutor::~ObjectDetectExecutor()
{
  status_list_.clear();
}

bool ObjectDetectExecutor::enable(int height, int width, CameraStatus & cam_status)
{
  ...
  if (!ret) {
    std::cout << "[ObjectDetectExecutor]: please add this camera name in config file: camera_"
  ...
  ret = get_yolo_object_det_post_process_node_info(postprocess_info);
  if (!ret) {
    cb_manager_->send_task_result({}, {},
        "please add nn config in config file due to object detect need "
        "it",
  ...
  std::cout << "[ObjectDetectExecutor]: send create pipeline request" << std::endl;
  if (!ret) {
  ...
  ret = cb_manager_->send_pipeline_request(TaskCommand::PIPELINE_LOAD, pipeline_id_,
      component_infos, std::vector<int>(), topic_name, pipeline);
  std::cout << "[ObjectDetectExecutor]: send pipeline load request" << std::endl;
  if (!ret) {
  ...
  std::cout << "[ObjectDetectExecutor]: camera topic name " << status.topic_name << std::endl;
  ...
  status.pipeline_id = pipeline_id_;
  status_list_.push_back(status);
  std::cout << "[ObjectDetectExecutor]: resize topic name " << status.topic_name << std::endl;
  ...
  status.pipeline_id = pipeline_id_;
  status_list_.push_back(status);
  std::cout << "[ObjectDetectExecutor]: color_convert topic name " << status.topic_name
            << std::endl;

  status.node_name = "detect_output";
  ...
  status.pipeline_id = pipeline_id_;
  status_list_.push_back(status);
  std::cout << "[ObjectDetectExecutor]: postprocess topic name " << status.topic_name << std::endl;

  // todo: add "image" source in the future
  node_name.push_back("detect_output");
...
bool ObjectDetectExecutor::stop(int height, int width, bool stop_camera)
{
  if (status_list_.size() == 0) {
    cb_manager_->send_task_result({}, {}, "please add this task first", false);
    std::cout << "[ObjectDetectExecutor]: please call enable first." << std::endl;
    return false;
  }
  if (listener_ > 1) {
    cb_manager_->send_task_result({}, {}, "", true);
    std::cout << "[ObjectDetectExecutor]: unregister success." << std::endl;
    listener_--;
    return false;
  }
  std::string topic_name;
...
void ObjectDetectExecutor::add_new_listener(int height, int width)
{
  std::cout << "[ObjectDetectExecutor]: add new listener." << std::endl;
  ...
  for (auto status : status_list_) {
    // todo: add "image" source in the future
    if (status.node_name != "detect_output") {
      continue;
    }
